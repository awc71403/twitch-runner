...
public class TwitchIRC : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        string anotherchannelName = $"{PlayerPrefs.GetString("ChannelOne")}";
        if (anotherchannelName != "")
        {
            ConnectAnotherServer(anotherchannelName);
        }

        anotherchannelName = $"{PlayerPrefs.GetString("ChannelTwo")}";
        if (anotherchannelName != "")
        {
            ConnectAnotherServer(anotherchannelName);
        }

        anotherchannelName = $"{PlayerPrefs.GetString("ChannelThree")}";
        if (anotherchannelName != "")
        {
            ConnectAnotherServer(anotherchannelName);
        }

        anotherchannelName = $"{PlayerPrefs.GetString("ChannelFour")}";
        if (anotherchannelName != "")
        {
            ConnectAnotherServer(anotherchannelName);
        }
    ...
    public void ConnectAnotherServer(string anotherchannelName)
    {
        if (!connectedChannels.Contains(anotherchannelName))
        {
            SendCommand("JOIN #" + anotherchannelName);
            Debug.Log(string.Format("Connecting to {0}", anotherchannelName));
            connectedChannels.Add(anotherchannelName);
        }
        else
        {
            Debug.Log(string.Format("Already connected to {0}", anotherchannelName));
        }
    }
}
